...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleConnection(client: Socket) {
    // called automatically when frontend establish websocket connection
...
send_message_chan(client: any, payload: any): void {
  ...
  send_message_dm(client: any, payload: any): void {
    ...
    if (dm_name) {
      this.server
        .to(this.chatService.find_id(username))
        .emit("priv_message", { message, sender: client.id });
...
async leave_room(client: any, room_name: string) {
  ...
  add_priv_user(client: any, payload: any) {
    ...
    const user = this.chatService.find_user_with_id(client.id);
    if (
      this.chatService.is_admin(user.userName, room_name) ||
      this.chatService.is_owner(user.userName, room_name)
    ) {
